...
RSpec.describe UserMailer, type: :mailer do
  ...
  describe 'notify_broadcast_creators_on_broadcast_creation' do
    let(:user) { create(:user) }
    let(:mail) { UserMailer.notify_broadcast_creators_on_broadcast_creation }
    before { create(:broadcast, creator_id: user.id)}

    it 'renders headers' do
      expect(mail.subject).to eq 'A broadcast has been created'
      expect(mail.from).to eq ['info@rundfunk-mitbestimmen.de']
      expect(mail.to).to eq [user.email]
    end
